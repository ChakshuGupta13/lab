...
Headline:
  `theorem2_kyber_statement` (closes the project's only `sorry`).
-/
...
theorem teleTerm_basis_freshIdx_lower_eq_zero [KyberLike spec]
    (Z : spec.Twiddles K)
...
/-- A combined input from the per-layer fresh-index basis vectors.
    ...
    Assembly argument. -/
noncomputable def combinedFreshBasis [KyberLike spec]
...
lemma teleTerm_combinedFreshBasis [KyberLike spec]
    (Z : spec.Twiddles K)
    {F : spec.FaultSet} (hF : spec.OneFaultPerLayer F)
...
lemma faultDiff_combinedFreshBasis [KyberLike spec]
...
/-- **Step 3a — coefficient annihilation.** If `D_K (combinedFreshBasis c) = 0`,
    ...
    diagonal). -/
theorem combinedFreshBasis_injective [KyberLike spec]
    {Z : spec.Twiddles K} (hZ : ∀ ℓ g, Z ℓ g ≠ 0) (h2 : (2 : K) ≠ 0)
    {F : spec.FaultSet} (hF : spec.OneFaultPerLayer F)
    (c : ∀ (k : Fin spec.N), Fin (spec.L k) → K)
...
theorem faultDiff_basis_freshIdx_linearIndependent [KyberLike spec]
    {Z : spec.Twiddles K} (hZ : ∀ ℓ g, Z ℓ g ≠ 0) (h2 : (2 : K) ≠ 0)
    {F : spec.FaultSet} (hF : spec.OneFaultPerLayer F) :
    LinearIndependent K
...
lemma card_sigma_fresh [KyberLike spec] :
...
theorem faultDiff_rank_ge_sum_L [KyberLike spec]
    {Z : spec.Twiddles K} (hZ : ∀ ℓ g, Z ℓ g ≠ 0) (h2 : (2 : K) ≠ 0)
    {F : spec.FaultSet} (hF : spec.OneFaultPerLayer F) :
