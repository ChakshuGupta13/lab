// 38-step SFS head-to-head: Table 1 (sparse SP) vs Table 5 (full characteristic).
//
// SFS, SKIP encode_iv. Paper's hardest result (8 hours single CPU on the
// authors' hardware).
//
...
static void encode_sfs(SHA256Encoder& enc, const CharState& sp, int n_steps) {
    // SFS: NO encode_iv(). DASH on IV positions forces inter-copy equality.
...
static void solve_one(const CharState& sp, int64_t conflict_limit, const char* label) {
    printf("\n=== %s ===\n", label);
    SHA256Encoder enc(38);
    encode_sfs(enc, sp, 38);
    ...
    if (status == 10) {
        ...
        bool ok = verify_sfs(IV_x, m1, m2, 38);
...
int main(int argc, char* argv[]) {
    ...
    printf("=== 38-step SFS: Table 1 (sparse SP) vs Table 5 (full characteristic) ===\n");
    printf("conflict_limit=%lld\n\n", (long long)cl);

    CharState t1 = mendel2013::starting_point_38_sfs();
    CharState t5 = mendel2013::starting_point_38_sfs_full();
    count_slack(t1, "Table 1 (sparse SP):", 38);
    count_slack(t5, "Table 5 (full):",      38);

    solve_one(t1, cl, "Table 1 (sparse SP)");
    solve_one(t5, cl, "Table 5 (full characteristic)");
    return 0;
}
